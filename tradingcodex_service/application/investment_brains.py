...
from tradingcodex_service.application.managed_package_sources import (
    git_tree_records as managed_git_tree_records,
    is_remote_git_location as managed_is_remote_git_location,
    materialized_package_source,
    run_git as managed_run_git,
    resolve_workspace_source_locator as managed_resolve_workspace_source_locator,
    tracked_source_metadata as managed_tracked_source_metadata,
    validate_git_location as managed_validate_git_location,
    validate_public_https_url,
    validate_workspace_source_locator as managed_validate_workspace_source_locator,
)


def _run_git(args: list[str]) -> None:
    """Preserve the Brain service's tested safe-Git boundary after helper extraction."""

    managed_run_git(args, label="investment brain")
...
def _materialized_source(
    *,
    ...
    with materialized_package_source(
        local_source=local_source,
        git_source=git_source,
        ref=ref,
        label="investment brain",
        temporary_prefix="tradingcodex-brain-git-",
        checkout_paths=(BRAIN_MANIFEST_PATH.parts[0], BRAIN_SKILL_PATH.as_posix()),
...
def _git_tree_records(
    ...
    *,
    ...
    return managed_git_tree_records(
        checkout,
        recursive=recursive,
        paths=(BRAIN_MANIFEST_PATH.parts[0], BRAIN_SKILL_PATH.as_posix()) if recursive else (),
...
def _validate_git_location(location: str) -> None:
    managed_validate_git_location(location, label="investment brain")


def _validate_public_repository(repository: str) -> None:
    validate_public_https_url(
        repository,
        label="investment brain declared repository",
    )
...
def _tracked_source_metadata(
    ...
    return managed_tracked_source_metadata(
        workspace_root,
        source_root,
        source,
        declared,
...
def _resolve_workspace_source_locator(workspace_root: Path, locator: str) -> Path:
    return managed_resolve_workspace_source_locator(
        workspace_root,
        locator,
        label="investment brain",
    )
...
def _validate_workspace_source_locator(locator: str) -> None:
    managed_validate_workspace_source_locator(locator, label="investment brain")


def _is_remote_git_location(location: str) -> bool:
    return managed_is_remote_git_location(location)
