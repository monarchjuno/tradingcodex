...
def test_decision_snapshot_postmortem_and_live_lesson_promotion_are_hash_bound(tmp_path: Path, capsys, monkeypatch) -> None:
    ...
    def create_episode(name: str) -> dict:
        ...
        call_mcp_tool(tmp_path, "create_research_artifact", {
            ...
            "summary": f"Bounded role evidence for {name}.",
            "status": {
                "handoff": "accepted",
                "evidence_readiness": "decision-grade",
                "action_readiness": "research-only",
                "confidence": "high",
                "confidence_basis": "Authenticated bounded live-forward evidence.",
                "missing_evidence": [],
        ...
        artifact = call_mcp_tool(tmp_path, "create_research_artifact", {
            ...
            "summary": f"Bounded decision evidence for {name}.",
            "status": {
                "handoff": "accepted",
                "evidence_readiness": "decision-grade",
                "action_readiness": "research-only",
                "confidence": "high",
                "confidence_basis": "Authenticated role evidence with explicit limits.",
                "missing_evidence": [],
                "blocked_actions": ["order", "execution"],
            },
            "lineage": {
            ...
            "requirements": [],
        }, transport_principal="head-manager")["artifact"]
    ...
    with pytest.raises(ValueError, match="forecast belongs to another workflow run"):
        record_decision_snapshot(tmp_path, {
            ...
            "decision_artifact_path": second["artifact"]["path"],
    ...
    with pytest.raises(ValueError, match="forecast knowledge_cutoff exceeds decision cutoff"):
        record_decision_snapshot(tmp_path, {
            ...
            "decision_artifact_path": first["artifact"]["path"],
            "forecast_ids": [first["forecast_id"]],
